...
save_base = "/homes/pr450/repos/research_projects/sampling_ddpm/results_link_sampler/ddpm_cued_residual_21_12_24/run"
...
residual_model = VectoralResidualModel(
    state_space_size, recurrence_hidden_layers, input_size, time_embedding_size
)
ddpm_model = DDPMReverseProcess(
    state_space_size, residual_model, input_model, sigma2x_schedule, time_embedding_size
).to("cuda")
...
for t in tqdm(range(num_trials)):
    ...
    network_inputs = torch.cat(
        [
            torch.nn.functional.one_hot(
                training_sample_set["network_inputs_idx"].to(device).to(torch.int64),
    ...
    if (t + 1850) % logging_freq == 0:
        ...
        novel_samples_dict = ddpm_model.generate_samples(
            ...
            samples_shape=(
                batch_size,
            ),  # if isinstance(ddpm_model, DDPMReverseProcess) else None,
        ...
        projected_trajs = (
            sample_trajectory[:5] @ ddpm_model.linking_matrix.T.detach().cpu().numpy()
        )
        for ex in range(5):
            axes[1, 0].plot(*projected_trajs[ex].T, color="green", alpha=0.1)

        near_end_step_samples = (
            sample_trajectory[:, -2]
            @ ddpm_model.linking_matrix.T.detach().cpu().numpy()
        )
        axes[1, 1].scatter(
            near_end_step_samples[:, 0],
            near_end_step_samples[:, 1],
            c=training_sample_set["network_inputs_idx"],
            alpha=0.5,
            s=1,
        )
