...
sigma2x_schedule = (
    (ultimate_sigma - starting_sigma)
    * 0.5
    * (1.0 - torch.linspace(0, torch.pi, num_timesteps).cos())
)
...
dc_residual_model = DoublyConditionedResidualModel(
    state_space_size, recurrence_hidden_layers, 2, time_embedding_size
)
dc_ddpm_model = DoublyConditionedDDPMReverseProcess(
    state_space_size,
    dc_residual_model,
    dc_input_model,
    sigma2x_schedule,
...
plt.plot(
    dc_ddpm_model.sigma2xt_schedule.cpu().numpy(), label="sigma2xt_schedule", alpha=0.4
)
plt.plot(dc_ddpm_model.a_t_schedule.cpu().numpy(), label="a_t_schedule", alpha=0.4)
plt.plot(
    dc_ddpm_model.root_b_t_schedule.cpu().numpy(), label="root_b_t_schedule", alpha=0.4
)
plt.plot(
    dc_ddpm_model.noise_scaler_schedule.cpu().numpy(),
    label="noise_scaler_schedule",
    alpha=0.4,
)
plt.plot(
    dc_ddpm_model.base_samples_scaler_schedule.cpu().numpy(),
    label="base_samples_scaler_schedule",
    alpha=0.4,
)
plt.plot(
    dc_ddpm_model.residual_scaler_schedule.cpu().numpy(),
    label="residual_scaler_schedule",
    alpha=0.4,
)
plt.plot(
    dc_ddpm_model.final_mean_scaler_schedule.cpu().numpy(),
    label="final_mean_scaler_schedule",
    alpha=0.4,
)
plt.legend()
...
for t in tqdm(range(num_trials)):
    ...
    if t >= 0:
        training_sample_set = embed_2D_items_in_clifford(
            batch_size, torus_directions, num_items, stim_radius
        )  # [batch_size, 2]
        y_samples = training_sample_set["y_samples"].to(
            device=device, dtype=sigma2x_schedule.dtype
        )

    noised_examples = dc_ddpm_model.noise(
        y_samples=y_samples, final_mean=training_sample_set["repeated_embedded_mean"]
    )
    training_info = dc_ddpm_model.residual(
        x_samples=noised_examples["x_t"],
        final_mean=training_sample_set["repeated_embedded_mean"]
        .unsqueeze(-2)
        .repeat(1, dc_ddpm_model.T, 1),
    ...
    if (t + 1850) % logging_freq == 0:
        ...
        sample_trajectory = (
            novel_samples_dict["sample_trajectory"].detach().cpu().numpy()
        )
        ...
        axes[0, 0].scatter(
            *training_sample_set["y_samples"].T,
            c=training_sample_set["network_inputs_idx"],
        )
        ...
        for m in range(num_items):
            axes[0, 1].scatter(
                *(
                    training_sample_set["base_samples"]
                    @ torus_directions["probe_directions"].T
                )[:, [2 * m, 2 * m + 1]]
                .T.cpu()
                .numpy()
            )
            axes[0, 2].scatter(
                *(
                    training_sample_set["base_samples"]
                    @ torus_directions["report_directions"].T
                )[:, [2 * m, 2 * m + 1]]
                .T.cpu()
                .numpy()
            )
        ...
        projected_trajs = (
            sample_trajectory[:5]
            @ dc_ddpm_model.linking_matrix.T.detach().cpu().numpy()
        )
        for ex in range(5):
        ...
        torch.save(
            dc_ddpm_model.state_dict(),
            os.path.join(save_base, f"state_T{num_timesteps}.mdl"),
        )
