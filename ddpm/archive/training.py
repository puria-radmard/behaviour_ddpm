...
data_generation_function, manifold_x, manifold_y, stimulus_shape = (
    MANIFOLD_INFORMATION_DICTIONARY[task_name]
)
...
residual_model = VectoralResidualModel(
    ...
    stimulus_representation_size,
    time_embedding_size,
)
ddpm_model = DDPMReverseProcess(
    16, residual_model, input_model, sigma2x_schedule, time_embedding_size
).to("cuda")
...
for t in tqdm(range(num_trials)):
    ...
    if t >= 0:
        y_samples, network_inputs, network_input_classes = data_generation_function(
            batch_size, True
        )  # [batch_size, 2]
    ...
    if (t + 1850) % logging_freq == 0:
        ...
        with torch.no_grad():
            ...
            novel_samples = (
                novel_samples_dict["behaviour_samples"].detach().cpu().numpy()
            )
            sample_trajectory = (
