...
class DoublyConditionedResidualModel(ResidualModel):
    ...
    def __init__(
        ...
        self.layers.append(
            nn.Linear(
                2 * state_space_size + self.input_size + self.time_embedding_size,
                state_space_size,
            )
        )
    ...
    def forward(
        self, x: _T, final_mean: _T, t_embeddings_schedule: _T, input_vector: _T
    ) -> _T:
        """
...
class DoublyConditionedDDPMReverseProcess(DDPMReverseProcess):
    ...
    def __init__(
        ...
        self.final_mean_scaler_schedule = (
            (
                (1.0 - self.sigma2xt_schedule - self.a_t_schedule.square())
                / (self.root_b_t_schedule.square() * (1 - self.sigma2xt_schedule))
            )
            - (1.0 / (1 - self.sigma2xt_schedule).sqrt())
            - (
                (self.sigma2xt_schedule * self.a_t_schedule * (1 - self.a_t_schedule))
                / (
                    (1 - self.a_t_schedule.square())
                    * (1 - self.sigma2xt_schedule).sqrt()
                )
            )
        )

    def to(self, *args, **kwargs):
        self.final_mean_scaler_schedule = self.final_mean_scaler_schedule.to(
            *args, **kwargs
        )
    ...
    def noise(self, y_samples: _T, final_mean: _T) -> Dict[str, _T]:
        ...
        x_t_means = (reshaped_a_t_schedule * x_0) + (
            1.0 - reshaped_a_t_schedule
        ) * final_mean.unsqueeze(-2)
        x_t = (
            x_t_means
            + self.root_b_t_schedule[*[None] * extra_dims].unsqueeze(-1) * epsilon
        )
    ...
    def generate_samples(
        ...
        network_input: _T,
        final_mean: _T,
        samples_shape: Optional[int] = None,
        base_samples: Optional[_T] = None,
        turn_off_noise: bool = False,
    ) -> Dict[str, _T]:
        """
        ...
        sample_trajectory = []
        noises = torch.randn(
            *samples_shape, self.T, self.hidden_size, device=base_samples.device
        )
        if turn_off_noise:
            noises *= 0.0
        ...
        for t_idx in range(1, self.T):
            ...
            new_residual = self.residual_model(
                base_samples, final_mean_estimate, t_embedding, input_vector
            )  # [..., 1, dim x]
