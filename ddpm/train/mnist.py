...
vae_base_path = args.vae_base_path
z_dim = args.z_dim
lr = args.lr
...
# Set up task
task_variable_gen_kwargs = args.dict.pop("task_variable_gen_kwargs").dict
task = getattr(mnist, args.task_name)(
    vae_base_path=vae_base_path, z_dim=z_dim, **task_variable_gen_kwargs
)
...
ddpm_model: MultiPreparatoryLinearSubspaceTeacherForcedDDPMReverseProcess = getattr(
    mnist, args.model_name
)(
    z_dim=z_dim,
    **model_config.dict,
...
for t in tqdm(range(num_trials)):
    ...
    residual_mse = task.sample_gen.mse(
        epsilon_hat_dict["epsilon_hat"], forward_process["epsilon"]
    )  # [batch, samples, time]
    ...
    if (t - plotting_offset) % logging_freq == 0:
        ...
        class_probs = trial_information.task_variable_information[
            "class_probabilities"
        ][0].tolist()
        axes[0, 0].set_title(
            f"Real samples from VAE\nclass probabilities = {class_probs}"
        )
        axes[0, 1].set_title("Generated samples from RNN")

        task.sample_gen.display_samples(
            trial_information.sample_information.sample_set[[0], :16], axes[0, 0]
        )
        task.sample_gen.display_samples(novel_samples_dict["samples"], axes[0, 1])

        axes[1, 0].set_title("Individual timesteps MSE")
        axes[1, 1].set_title("MSE averaged over timesteps")
        if t > plotting_start:
        ...
        axes[1, 2].set_title("Prep state behaviour activity regulariser")
        if t > plotting_start:
            axes[1, 2].plot(all_prep_state_losses[: t + 1 - plotting_start])
