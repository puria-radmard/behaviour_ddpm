...
class DendriticBranchingBlock(nn.Module):
    """
    ...
    def preactivation(self, activations: _T, t_embeddings_schedule: _T) -> List[_T]:
        ...
        preact_dendrite = torch.einsum('...n,nb->...nb', activations, self.propagation_weights)                 # n = nodes, b = branching factor
        # XXX: preact_dendrite = torch.einsum('...n,nb->...b', activations, self.propagation_weights).unsqueeze(-1)                 # n = nodes, b = branching factor * num nodes              XXX: see above! 
        ...
        return preact_dendrite, preact_time
    ...
    def forward(self, activations: _T, *other_inputs: _T) -> _T:
        ...
        """
        preact_dendrite, *preac_externals = self.preactivation(activations, *other_inputs)
        preact = preact_dendrite.reshape(*preact_dendrite.shape[:-2], self.num_nodes * self.branching_factor)
...
class DendriticBranchingBlockWithInput(DendriticBranchingBlock):
    ...
    def preactivation(self, activations: _T, t_embeddings_schedule: _T, input_vector: _T) -> _T:
        ...
        preact_dendrite, preact_time = super().preactivation(activations, t_embeddings_schedule)
        ...
        return preact_dendrite, preact_time, preact_input
...
class DendriticResidualModel(nn.Module):
    ...
    def __init__(
        ...
        tree_layers: List[DendriticBranchingBlockWithInput | DendriticBranchingBlock] = [DendriticBranchingBlockWithInput(state_space_size, branching_factors[0], time_embedding_size, input_size)]
        for bf in branching_factors[1:]:
            tree_layers.append(DendriticBranchingBlock(tree_layers[-1].output_size, bf, time_embedding_size))
        self.dendritic_tree: List[DendriticBranchingBlockWithInput | DendriticBranchingBlock] = nn.ModuleList(tree_layers)
    ...
    def forward(self, x: _T, t_embeddings_schedule: _T, input_vector: _T) -> _T:
        ...
        """
        node_activation = self.dendritic_tree[0].forward(x, t_embeddings_schedule, input_vector)
        for tree_layer in self.dendritic_tree[1:]:
