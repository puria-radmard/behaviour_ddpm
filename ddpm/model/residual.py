...
class VectoralResidualModel(nn.Module):
    ...
    def __init__(self, state_space_size: int, recurrence_hidden_layers: List[int], input_size: int, time_embedding_size: int, nonlin_first: bool) -> None:
        ...
        recurrence_layers = []
        recurrence_layers.extend([nn.Linear(state_space_size + time_embedding_size + input_size, all_layer_sizes[0]), nn.Softplus()])        # 1 to include time also!
