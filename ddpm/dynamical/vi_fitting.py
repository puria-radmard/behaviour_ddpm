...
from ddpm.dynamical.connectivity_distn import GaussianConnectivityDistribution, DiagonalGaussianConnectivityDistribution, PointMassConnectivityDistribution
from ddpm.dynamical.moment_pred import plot_ellipse
from ddpm.dynamical.vi_fitting_utils import calculate_elbo, generate_data_inner, generate_target_data
...
if __name__ == '__main__':
    ...
    for epoch in tqdm(range(args.num_epochs)):
        ...
        # Generate target data u* ~ p*(u | W*, h) [batch, neuron]
        with torch.no_grad():
            u_target = generate_target_data(
                target_dist, h_batch, args.timesteps, args.tau, args.dt, args.k, args.n, Sigma_xi, False, device
            )
        
        # Compute ELBO and gradients
        elbo, llh, kl = calculate_elbo(
            variational_dist, prior_dist, h_batch, u_target, 
            args.K, args.timesteps, args.tau, args.dt, args.k, args.n, Sigma_xi, kl_beta, device
        )

        with torch.no_grad():
            max_elbo, max_llh, _ = calculate_elbo(
                target_dist, prior_dist, h_batch, u_target, 
                args.K, args.timesteps, args.tau, args.dt, args.k, args.n, Sigma_xi, 0.0, device
            )
        ...
        print(f'Epoch {epoch}, ELBO: {elbo.item():.4f}, Max ELBO {max_elbo.item():.4f}')
        ...
        if epoch % 10 == 0:
            ...
            with torch.no_grad():
                ...
                # Covariance over args.K dimension: [B, T, N, N]
