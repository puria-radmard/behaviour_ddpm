...
class ExampleSampleGenerator(ABC):
    ...
    def mse(self, epsilon_hat: _T, epsilon: _T):
        ...
        *extra_dims, T = epsilon_hat.shape[: -len(self.sample_shape)]
        mse = (
            torch.square(epsilon_hat - epsilon).reshape(*extra_dims, T, -1).mean(-1)
        )  # [..., T]
...
class VectoralEmbeddedExampleSampleGenerator(ExampleSampleGenerator):
    ...
    def __init__(
        ...
        if self.residual_in_behaviour_plane_only:
            raise Exception(
                "are you sure you want to do this and not use LinearSubspaceTeacherForcedDDPMReverseProcess...?"
            )
    ...
    def generate_sample_set(
        ...
        selected_item_idx = torch.multinomial(
            selection_pmf, num_samples, replacement=True
        )  # [batch, sample]

        batch_idx = torch.arange(
            batch_size, device=variable_dict[self.response_location_key].device
        )[:, None].expand(
            batch_size, num_samples
        )  # [batch, sample]
        selected_cartesian = variable_dict[self.response_location_key][
            batch_idx, selected_item_idx
        ]  # [batch, sample, 2]
    ...
    def generate_sample_diagnostics(
        self, sample_set: _T, variable_dict: Dict[str, _T]
    ) -> Tuple[_T, Dict[str, _T]]:
        import pdb

        pdb.set_trace(header="get pmf by taking closest to report_feature")
    ...
    def mse(self, epsilon_hat: _T, epsilon: _T):
        ...
        return super(VectoralEmbeddedExampleSampleGenerator, self).mse(
            epsilon_hat, epsilon
        )
...
class RadialVectoralEmbeddedExampleSampleGenerator(
    ...
    def __init__(
        ...
        super().__init__(
            sample_size, sample_radius, residual_in_behaviour_plane_only, device
        )
    ...
    def generate_sample_set(
        ...
        batch_idx = torch.arange(
            batch_size, device=variable_dict["report_features"].device
        )[:, None].expand(
            batch_size, num_samples
        )  # [batch, sample]
        ...
        sample_abs = (
            self.sample_radius
            * torch.randn(batch_size, num_samples, 2, device=batch_idx.device)
            .square()
            .sum(-1)
            .sqrt()
        )  # [batch, sample]
        selected_angles = variable_dict["report_features"][
            batch_idx, selected_item_idx
        ]  # [batch, sample]
        sample_angles = (
            torch.rand_like(selected_angles) * self.sample_radius * 0.1
        ) + selected_angles
        cartesian_samples = torch.polar(sample_abs, sample_angles.to(sample_abs.dtype))
        cartesian_samples = torch.stack(
            [cartesian_samples.real, cartesian_samples.imag], -1
        )
...
class WhiteNoiseStripExampleSampleGenerator(ExampleSampleGenerator):
    ...
    def generate_strip_image(
        image_size: int, strip_pixel_width: int, selected_item_ang: torch.Tensor
    ) -> torch.Tensor:
        """
        Claude 3.5 Sonnet
        ...
        positive_where = torch.logical_and(
            (angle_diff <= angular_width.unsqueeze(0) / 2), radii > strip_pixel_width
        )
    ...
    def generate_sample_set(
        ...
        selected_item_idx = torch.tensor(
            np.random.choice(
                selection_pmf.shape[0], size=num_samples, replace=True, p=selection_pmf
            )
        )
        selected_item_ang = torch.tensor(variable_dict["report_features"])[
            selected_item_idx
        ]

        noise_offset = self.generate_strip_image(
            image_size=self.sample_shape[-1],
            strip_pixel_width=self.strip_pixel_width,
            selected_item_ang=selected_item_ang,
        )

        white_noise_images = (noise_offset * 0.5) + (
            torch.randn_like(noise_offset) * 0.15
        )
    ...
    def display_early_x0_pred_timeseries(
        self, sample_set: _T, axes: Axes, cmap: ScalarMappable
    ) -> None:
        ...
        for cs_idx, cs_ext in zip(chosen_sample_indices, chosen_sample_extents):
            ...
            axes.scatter(
                [cs_ext[0] + 2], [cs_ext[-1] - 2], color=cmap.to_rgba(T - cs_idx), s=20
            )
        axes.plot(
            [-sample_set.shape[-1], sample_set.shape[-1]], [0.0, 0.0], color="red"
        )
        axes.plot(
            [0.0, 0.0], [-sample_set.shape[-1], sample_set.shape[-1]], color="red"
        )
