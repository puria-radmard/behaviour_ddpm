...
class WMDiffusionTask:
    ...
    def generate_trial_information(self, batch_size: int, num_samples: int, **task_variables_kwargs) -> TrialInformation:
        """
        ...
        """
        task_variable_information = self.task_variable_gen.generate_variable_dict(batch_size = batch_size, **task_variables_kwargs)
        network_inputs = self.sensory_gen.generate_sensory_inputs(task_variable_information)
        network_inputs = network_inputs.unsqueeze(1).expand(batch_size, num_samples, *self.sensory_gen.sensory_shape)
